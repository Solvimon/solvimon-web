...
function setup({
    ...
    mockReadFileSync.mockReturnValueOnce(JSON.stringify(supported) as never);
...
describe('generateTranslationsReport', () => {
    ...
    it('includes the translations-report marker', () => {
        setup({ source: {}, supported: ['en-US'], locales: { 'en-US': {} } });

        expect(generateTranslationsReport(defaultArgs)).toContain('<!-- translations-report -->');
    });
    ...
    it('shows NOTE when all locales are complete', () => {
        ...
        const report = generateTranslationsReport(defaultArgs);
    ...
    it('uses plural form when multiple locales have missing keys', () => {
        ...
        expect(generateTranslationsReport(defaultArgs)).toContain(
    ...
    it('uses plural "keys" for multiple missing keys', () => {
        ...
        expect(generateTranslationsReport(defaultArgs)).toContain('2 keys missing');
    });
    ...
    it('treats a non-existent locale file as fully missing', () => {
        ...
        mockReadFileSync
            .mockReturnValueOnce(JSON.stringify({ greeting: 'Hello' }) as never)
            .mockReturnValueOnce(JSON.stringify(['nl-NL']) as never);
    ...
    it('includes the SHA in the footer', () => {
        ...
        expect(generateTranslationsReport({ ...defaultArgs, sha: 'deadbeef' })).toContain(
            'Measured at deadbeef',
        );
    });

    it('shows NOTE when source has no keys', () => {
        setup({ source: {}, supported: ['en-US', 'nl-NL'], locales: { 'en-US': {}, 'nl-NL': {} } });

        expect(generateTranslationsReport(defaultArgs)).toContain('> [!NOTE]');
    });
});
