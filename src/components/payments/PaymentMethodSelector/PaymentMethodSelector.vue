...
<template>
    <div class="sv-payment-method-selector grid grid-cols-1 gap-2" :class="rootClass">
        <EmptyStatePlaceholder
            v-if="!hasPaymentMethods && !canAddPaymentMethod"
            class="sv-payment-method-selector__empty"
            ...
            <template #title>
                {{
                    $t({
                        ...
                        description:
                            'Shown in place of the payment method choice when the customer has nothing stored and nothing can be added',
                        id: 'payment_method_selector.empty_state.title',
                    })
                }}
            </template>
        </EmptyStatePlaceholder>
